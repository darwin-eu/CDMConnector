...
cdm$cohort2 <- cdm$cohort %>% 
  ...
  compute_query(name = "cohort2", temporary = FALSE, schema = attr(cdm, "write_schema"), overwrite = TRUE) %>% 
  new_generated_cohort_set()

cdm$cohort2

cohort_count(cdm$cohort2)
...
cdm$cohort2 %>% 
  ...
  summarize(mean_days_in_cohort = mean(days_in_cohort)) %>% 
  collect() %>% 
