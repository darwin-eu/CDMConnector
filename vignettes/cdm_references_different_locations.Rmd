---
title: "cdm references in different locations"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{cdm_references_different_locations}
...
cdm <- cdm_from_con(db,
  cdm_tables = tidyselect::all_of(
    c("person", "condition_occurrence")
  )
)
...
cdm$person <- cdm$condition_occurrence %>%
  select("person_id") %>%
  left_join(cdm$person %>% select("person_id", "year_of_birth"),
    by = "person_id"
  )
cdm$person %>% tally()
```
...
cdm_arrow$result <- cdm_arrow$person %>%
  left_join(cdm_arrow$condition_occurrence,
    by = "person_id"
  ) %>%
