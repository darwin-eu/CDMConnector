...
test_that("cohort generation works on duckdb", {
  ...
  cdm <- generateCohortSet(cdm, cohortSet, name = "cohort", computeAttrition = TRUE)
  expect_true("cohort" %in% listTables(con, schema = write_schema))

  df <- cdm$cohort %>% head() %>% dplyr::collect()
  expect_s3_class(df, "data.frame")
  expect_true(all(names(df) == c("cohort_definition_id", "subject_id", "cohort_start_date", "cohort_end_date")))

  expect_s3_class(dplyr::collect(attrition(cdm$cohort)), "data.frame")
  expect_s3_class(dplyr::collect(settings(cdm$cohort)), "data.frame")
  expect_s3_class(dplyr::collect(cohortCounts(cdm$cohort)), "data.frame")

  # TODO better cleanup of tables

  # listTables(con, write_schema)

  DBI::dbRemoveTable(con, DBI::Id(schema = "main", table = "cohort"))
  expect_false("cohort" %in% listTables(con, schema = write_schema))
